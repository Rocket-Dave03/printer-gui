...
void gui_set_style_padding(struct GuiElement *self, struct Padding padding) { self->style.padding = padding; }

void gui_set_style_margin(struct GuiElement *self, struct Padding margin) { self->style.margin = margin; }

struct GuiSize gui_get_max_internal_size(struct GuiElement *self) {
	uint32_t width = self->_size.width - self->style.padding.left - self->style.padding.right;

	uint32_t height = self->_size.height - self->style.padding.top - self->style.padding.bottom;

	return (struct GuiSize){width, height};
}

void gui_show(struct GuiElement *self, struct Buffer *buf, uint32_t x, uint32_t y) {
	write_buffer(buf, self->buf, x + self->style.margin.left, y + self->style.margin.top);
	if (self->layout != NULL) {
		layout_show(self->layout, buf, x + self->style.margin.left + self->style.padding.left,
